...
const Profile = () => {
  ...
  return (
    ...
    <>
        <Navbar/>
        <div className=" mx-auto rounded-xl  grid grid-cols-5 gap-4 p-6 w-[99%] sm:w-[70%] md:[60%] mt-8 bg-gray-100">
          
          {isUserInfoReady && (
            <div className="col-span-4 gap-6  grid grid-cols-2 px-4 ">
            <ReqFieldItem  label="Name" value={user["userDetails"].Username !== "" ?  user["userDetails"].Username : "Add Name edit btn"} />
            <ReqFieldItem  label="Position" value={user["userDetails"].Role} />
            <ReqFieldItem  label="Email" value={user["userDetails"].email} />
            <ReqFieldItem label="Organization" value={user["userDetails"].organization} />          
          </div>
          )}
